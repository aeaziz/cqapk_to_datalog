...
from cqapk_to_datalog.algorithms import apply_renaming_to_atom, apply_renaming_to_atom_values, generate_renaming, generate_new_variables
from typing import List, Set, Tuple, Dict

def generate_z_and_c(x: List[AtomValue], y: List[AtomValue], vars_y: List[AtomValue], index: int) -> Tuple[
...
class FO_RewritingQuery(DatalogQuery):
    def __init__(self, head, done):
...
class RewriteAtomQuery(FO_RewritingQuery):
    def __init__(self, data: RewritingData):
        ...
        FO_RewritingQuery.__init__(self, head, data.done)
...
class BadBlockQuery(FO_RewritingQuery):
    def __init__(self, data: RewritingData):
...
class GoodFactQuery(FO_RewritingQuery):
    def __init__(self, data: RewritingData):
        ...
        for val in data.c:
            ...
            ea.released = set(data.frozen + data.vars_x + data.vars_z).intersection(set([val, data.c[val]]))
            self.add_atom(ea)
        if not data.is_last:
